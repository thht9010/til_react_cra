...
- props 는 개겣라서 `객체 구조 분해 할당` 해서 사용할 수 있음.
...
function Slide() {
  ...
  return (
    <div style={{ padding: "30px" }}>
      ...
      <br />
      <br />
      <BasicUpButton>CSS 적용버튼</BasicUpButton>
      <br />
      <br />
      <Box bg="red" width="200px" b="5" cc="orange">
        첫번째 박스
      </Box>
...
- 최종 버전 (components/ui/Button.jsx)
...
function Avator({
  ...
  `;
  // jsx 자리
  return <StlyeAvator src={src} alt={alt} size={size} shadow={shadow} />;
...
function Alert({ children, type = "deafult" }) {
  // js 자리
...
function ProgressBar({ percent = 0, color = "#000000" }) {
  // js 자리
...
- fashly 한 값 종류: `false, null, undefinded, 0, "", NaN`
- if문
...
- `JS 자리에` if 문과 switch 문을 사용할 수 없습니다.

- `JSX 자리에` if 문과 switch 문을 사용할 수 없습니다.
- if 문과 switch 문 대신에 `3 항 연산자` 가 가장 많이 사용됨.
- `JSX 자리에` falshy 한 값은 출력이 안됩니다.
...
function Hi(isLogin) {
  ...
  //   }
  // jsx 자리
  return <div>{isLogin ? "반가워요" : "안녕하십니까"}안녕</div>;
}
function Test() {
  // js 자리
